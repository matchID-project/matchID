...
deces-backend     | dockerimage.yml / build         | ci.yml / deces-backend build docker image and tests | make -C packages/deces-backend DATA_DIR=data backend-build-image; make -C packages/deces-backend deploy-dependencies; make -C packages/deces-backend DATA_DIR=${GITHUB_WORKSPACE}/packages/deces-backend/data backend-test-vitest
...
- Le job lourd upstream `bulk` / artillery (`backend-perf-clinic`,
  `test-perf-v1`) n'est pas ajouté à la CI courte; il reste un point explicite à
  statuer avant le lot 9.
...
- `ci.yml` ne déclenche plus de push spécifique sur `feat/refacto-make`; les
  triggers cibles sont `push` sur `dev`/`master` et PR vers `dev`/`master`.
