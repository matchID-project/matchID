...
[ -z "${APP_FRONTEND}" -o -z "${BACKEND_PORT}" -o -z "${BACKEND_HOST}" -o -z "${BACKEND_PROXY_PATH}" ] && echo "missing some env var APP_FRONTEND: ${APP_FRONTEND} BACKEND_PORT: ${BACKEND_PORT} BACKEND_HOST: ${BACKEND_HOST} BACKEND_PROXY_PATH: ${BACKEND_PROXY_PATH}" && exit 1
